...
- **G28 Set-Cookie recovery on hosts lacking getSetCookie** — the accessor is absent under Hermes on both iOS and Android (`docs/evidence/HERMES_MULTI_COOKIE_JOIN.md`), and the host joins several `Set-Cookie` lines with `", "` — observed 2026-09-07 — so splitting on a bare comma fails where an `Expires` attribute is present; a rule splitting on a comma and optional whitespace followed by a cookie name and `=` recovered both cookies in that probe. The shipped fallback in `packages/transport` still stores the joined string whole, as one jar entry, because a cookie VALUE containing that same pattern would defeat the rule and no such value has been observed.
