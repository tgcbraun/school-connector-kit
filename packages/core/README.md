...
- **G9 connector fetcher return types unbound to schema concepts** — `ConnectorFetchers` is still declared `readonly unknown[]` in the contract; the WebUntis connector returns the narrower `readonly Assignment[]`, which is legal but not compelled. What a live run established is the binding of the concepts to real values — 12 rows mapped by the published connector validated against the committed Assignment schema without exception — not a narrowing of the contract's return type, which a second connector will meet unchanged (ADR-003).
