/**
 * Connector contract tests (ADR-003), plus the entry-closure rule and the
 * scanner's control cases.
 *
 * RULE (ADR-003, decision 9): nothing reachable from the package entry point
 * may use Node built-ins or DOM globals, because that closure is what a
 * React Native or Flutter-hosted bundle loads. Build-time modules are out
 * of scope by being unreachable, not by being named: there is no exclusion
 * list, and none should be able to drift.
 *
 * The scan starts at packages/core/src/index.ts and follows its transitive
 * import closure: import/export-from specifiers that are relative are
 * followed, with "./x.js" resolving to the sibling "./x.ts" source file;
 * bare specifiers (e.g. "zod") are not followed; node_modules is never
 * descended; each file is visited exactly once. A global *reference* means
 * a bare expression use (window.innerWidth, process.env); a declared
 * property name ("readonly window?: FetchWindow"), an object key, or a
 * member access ("request.window") is not a reference — the contract itself
 * mandates a field named "window" on FetchRequest. The identifier check
 * runs over code with comments and string contents blanked (offset
 * preserving), so prose and values cannot read as identifiers; the "node:"
 * check looks at string literals in an import / export-from / require
 * context.
 */
...
import type { Connector, ConnectorCapability } from "../src/index.js";
...
describe("capabilitiesOf", () => {
  it("returns only the fetcher keys that hold functions, in code-point order", () => {
    const connector = {
      platform: "test",
      fetchers: {
        // Insertion order is deliberately NOT sorted order, and one key
        // holds a non-function value: only function-holding keys survive,
        // and the output is code-point sorted.
        message: async () => [] as readonly unknown[],
        event: undefined,
        assignment: async () => [] as readonly unknown[],
      },
      authenticate: async () => {},
    } as unknown as Connector;

    expect(capabilitiesOf(connector)).toEqual(["assignment", "message"]);
  });

  it("returns an empty array for an empty fetcher map", () => {
    const connector = {
      platform: "test",
      fetchers: {},
      authenticate: async () => {},
    ...
    expect(capabilitiesOf(connector)).toEqual([]);
  });
});
...
describe("ConnectorError", () => {
  it("carries its code and preserves the caller-supplied message unmodified", () => {
    const fixed = "a fixed, caller-chosen message";
    const error = new ConnectorError("auth_failed", fixed);

    expect(error.code).toBe("auth_failed");
    expect(error.message).toBe(fixed);
...
describe("assertNoCookieHeaders", () => {
  ...
  it("accepts headers without cookie keys", () => {
    expect(() =>
      assertNoCookieHeaders(
        make({
  ...
  it("rejects 'Cookie', 'cookie', 'Set-Cookie' and 'SET-COOKIE'", () => {
    for (const key of ["Cookie", "cookie", "Set-Cookie", "SET-COOKIE"]) {
      let caught: unknown;
      try {
        assertNoCookieHeaders(make({ [key]: "session-token-xyz" }));
      } catch (err) {
        caught = err;
      }
      expect(caught, `expected a throw for header key ${JSON.stringify(key)}`).toBeInstanceOf(
        ConnectorError,
      );
  ...
  it("throws a message containing neither the key nor the value", () => {
    const key = "Set-Cookie";
    const value = "session-token-xyz";
    let caught: unknown;
    try {
      assertNoCookieHeaders(make({ [key]: value, "X-One": "2" }));
    } catch (err) {
      caught = err;
    }
...
// ---------------------------------------------------------------------------
// Negative pins, mirrored from the README gap register
// ---------------------------------------------------------------------------

describe("ConnectorCapability negative pins", () => {
  it("pins that 'absence' and 'assessment' are not assignable to ConnectorCapability", () => {
    // Type-level only; tsc --noEmit enforces it. If either literal were ever
    // added to ConnectorCapability, the @ts-expect-error directives below
    // would become unused and the typecheck would fail.
    //
    // @ts-expect-error — an unmodeled concept (README gap G0), not a capability
    const absence: ConnectorCapability = "absence";
    //
    // @ts-expect-error — an unmodeled concept (README gap G1), not a capability
    const assessment: ConnectorCapability = "assessment";

    // Runtime shape of the same pin: the capability set is exactly the four
    // normalized message concepts.
    const all = [
      "timetable_entry",
      "assignment",
      "event",
    ...
    expect(all).not.toContain("absence" as (typeof all)[number]);
    expect(all).not.toContain("assessment" as (typeof all)[number]);
    void absence;
    void assessment;
  });
});
...
// ---------------------------------------------------------------------------
// The scanner itself: shared primitives and the 11 control cases
// ---------------------------------------------------------------------------
 ...
 * that immediately precedes it — the import-specifier check needs the
 * literal and its context.
 */
