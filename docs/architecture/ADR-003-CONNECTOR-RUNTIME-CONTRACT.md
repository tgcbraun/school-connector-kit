...
**Status:** Accepted
**Supersedes:** nothing
**Related:** ADR-001 (local-first TypeScript), ADR-002 (HTML structural capture), `packages/core` (the schema package holding the Zod definitions for schema 0.1 that this contract builds on; the reviewed fixture corpus it consumes is the top-level `fixtures/` directory), `packages/core/README.md` (gaps G6, G9–G14)

---

## Context

`packages/core` now carries schema `0.1` — the normalized concepts the capture corpus established across WebUntis, DieSchulApp, and Kikom — but nothing that runs a platform. The next step is the first connector implementation. Which platform it targets is not settled by this ADR; what is settled is that it will be consumed in two places this repository does not control: a React Native application under Hermes, and an embedded JavaScript engine hosted by a Flutter application.
...
**Consequence:** a partial connector is a legal connector: a fetcher key that is absent or explicitly `undefined` yields no capability, and `capabilitiesOf` reports exactly the capabilities the connector implements — nothing more, nothing less.
...
**What a contributor must now write** (a working connector against this contract, in `packages/connectors/`):
